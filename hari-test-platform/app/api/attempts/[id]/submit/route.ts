...
export async function POST(
  ...
  try {
    ...
    // The kept payload is still in DISPLAYED terms (same as autosave), so it
    // needs the same conversion back to canonical option numbers.
    const mergeAnswers = stillInTime
      ? parsed.data.answers.map((a) => {
          const order = findOrder(attempt.optionOrders, a.questionNumber);
          return {
            questionNumber: a.questionNumber,
            selectedOption: order ? toOriginalOption(order, a.selectedOption) : a.selectedOption,
          };
        })
      : undefined;
