...
export default function AttemptDetailPage() {
  ...
  if (!attempt) {
    return <div className="p-4 text-muted-foreground sm:p-8">Attempt not found.</div>;
  }
      ...
      <div>
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-muted-foreground">{attempt.studentEmail}</p>
          </div>
      ...
      <div className="space-y-4">
        {breakdown.map((q) => (
          <Card key={q.questionNumber}>
            <CardHeader className="flex flex-col items-start gap-2 space-y-0 sm:flex-row sm:justify-between">
              <CardTitle className="text-base font-medium leading-relaxed">
                {q.questionNumber}. {q.question}
              </CardTitle>
              <Badge className={`shrink-0 ${q.isCorrect ? "bg-green-600" : "bg-red-600"}`}>
              ...
              {q.options.map((optionText, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 break-words">{optionText}</span>
                    {isSelected && (
